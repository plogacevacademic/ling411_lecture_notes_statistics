...
```{r eval=FALSE}
load( "./data_navarro/clinicaltrial.Rdata" )
who(TRUE)
...
```{r eval=FALSE}
load( "./data_navarro/parenthood.Rdata" )
who(TRUE)
...
```{r eval=F, parenthood, fig.cap="Histograms for the three interesting variables in the `parenthood` data set", echo=FALSE}
...
```{r eval=F, parenthoodtab, echo=FALSE}
knitr::kable(rbind(
...
```{r eval=F, scatterparent1a, fig.cap="Scatterplot showing the relationship between `dan.sleep` and `dan.grump`", echo=FALSE}
...
```{r eval=F, scatterparent1b, fig.cap="Scatterplot showing the relationship between `baby.sleep` and `dan.grump`", echo=FALSE}
oneCorPlot <- function(x,y,...) {
...
```{r eval=F, scatterparent2, fig.cap="Scatterplot showing the relationship between `baby.sleep` and `dan.sleep`", echo=FALSE}
oneCorPlot <- function(x,y,...) {
...
```{r eval=F, corr, fig.height=10, echo=FALSE, fig.cap="Illustration of the effect of varying the strength and direction of a correlation"}
...
```{r eval=F, anscombe, echo=FALSE, fig.cap="Anscombe's quartet. All four of these data sets have a Pearson correlation of $r = .816$, but they are qualitatively different from one another."}
par(mfcol = c(2, 2)) # Create a 2 x 2 plotting matrix

load("./data_navarro/anscombesquartet.Rdata")
...
width <- 7
	...
	# load data
	load("./data_navarro/effort.Rdata")
	require(lsr)
