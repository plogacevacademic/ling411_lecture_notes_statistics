...
dative_summary1 <- dative_relevant %>% group_by(LengthOfRecipient) %>% summarize(perc_np = mean(RealizationOfRecipient == "NP"), N = n())
ggplot(dative_summary1, aes(x= LengthOfRecipient, perc_np)) + geom_point(aes(size = N))

dative_summary2 <- dative_relevant %>% group_by(cat_LengthOfRecipient) %>% summarize(perc_np = mean(RealizationOfRecipient == "NP"), N = n())
p <- ggplot(dative_summary2, aes(x= cat_LengthOfRecipient, perc_np)) + geom_point()

coef(lm( perc_np ~ 1 + contr1_LengthOfRecipient, data = dative_summary2))

# Query to lm, with a model specification:
# perc_np = a*1 + b*contr1_LengthOfRecipient

# Answer: 
# perc_np = 0.8752244*1 + -0.4269152*contr1_LengthOfRecipient

p <- ggplot(dative_summary2, aes(x= contr1_LengthOfRecipient, perc_np)) + geom_point()
p + geom_abline(intercept = 0.8752244, slope = -0.4269152)
 


dative_summary2 <-
dative_relevant %>% group_by(cat_LengthOfTheme) %>% summarize(perc_def = mean(RealizationOfRecipient == "NP"), N = n() ) 

dative_summary2 %>% ggplot(aes(cat_LengthOfTheme, perc_def)) + geom_point()

dative_summary2$cLT <- dative_summary2$cat_LengthOfTheme %>% dplyr::recode("short"=0, "long"=1)

coef(lm(perc_def ~ 1 + cLT, data = dative_summary2))
a = 0.6821904   
b = 0.1436309 

dative_summary3 <-
dative_relevant %>% group_by(cat_LengthOfTheme, cat_LengthOfRecipient) %>% summarize(perc_def = mean(RealizationOfRecipient == "NP"), N = n() ) 
dative_summary2 %>% ggplot(aes(cat_LengthOfTheme, perc_def)) + geom_point()
...
dative_summary3 %<>% mutate( cLR = dplyr::recode(cat_LengthOfRecipient, "short"=-.5, "long"=.5),
                             cLT = dplyr::recode(cat_LengthOfTheme, "short"=-.5, "long"=.5)
                            )

coef(lm(perc_def ~ 1 + cLT + cLR + cLT*cLR, data = dative_summary3))

dative_summary3 %>% group_by(cLT) %>% summarize(perc_def = mean(perc_def))
